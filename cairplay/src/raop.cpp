...
struct raop_ctx_s *raop_create(uint32_t host, char *name,
	...
	struct raop_ctx_s *ctx = (raop_ctx_s*) malloc(sizeof(struct raop_ctx_s));
...
//	ctx->svc = mdnsd_register_svc(ctx->svr, id, "_raop._tcp.local", ctx->port, NULL, (const char**) txt);
	pthread_create(&ctx->thread, NULL, (void * _Nullable (* _Nonnull)(void * _Nullable)) &rtsp_thread, ctx);
...
// #ifdef WIN32
	...
	out = (unsigned char*) malloc(RSA_size(rsa));
	switch (mode) {
		case RSA_MODE_AUTH:
